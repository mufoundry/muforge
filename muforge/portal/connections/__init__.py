...
class BaseConnection:
    ...
    def __init__(self, app, link: ConnectionLink):
        self.app = app
        self.link = link
    ...
    def get_headers(self) -> dict[str, str]:
        ...
        out["User-Agent"] = f"muforge-portal/{self.link.info.client_protocol.lower()}"
        return out
    ...
    async def run(self):
        async with asyncio.TaskGroup() as tg:
            ...
            logger.info(
                f"Connection {self.link.info.connection_id} shutting down: {self.shutdown_cause}"
            )
    ...
    async def send_data(self, package: str, data: typing.Any):
        await self.link.outgoing_queue.put(LinkData(package=package, data=data))

    async def send_text(self, text: str):
        """
        Sends a text message to the client. It doesn't care much if it's plain text or has embedded ANSI.
        ANSI will probably be generated by Rich through .send_rich().
        """
        await self.send_data(package="Text.ANSI", data=text)
    ...
    async def handle_incoming_event(self, data):
        """
        Handles an incoming event from the link.
        """
        match data:
    ...
    def create_client(self):
        httpx_settings = self.app.settings["httpx"]

        config = dict()
        for k, v in httpx_settings.items():
            match k:
                case "limits":
                    config[k] = Limits(**v)
                case _:
